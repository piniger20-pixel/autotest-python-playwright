...
def test_forwarder_cancel_order_in_work(chromium_page: Page):
    ...
    # 3. Отменяем заказ заказчиком и проверяем детали отмены
    biz.cancel_order_by_customer_and_verify(page, order_id)
    
    # 4. Проверяем что рейс автоматически отменён
    biz.verify_flight_cancelled_after_order_cancellation(page, flight_id)
